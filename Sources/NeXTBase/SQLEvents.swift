...
public final class HookBox {
    ...
    var fn: (NeXTBase?, RowUpdate) -> Void
    
    public init(ndb: NeXTBase, fn: @escaping (NeXTBase?, RowUpdate) -> Void) {
        self.ndb = ndb
        self.fn = fn
    }
    ...
    func callAsFunction(op: Int32, db: UnsafeString?, table: UnsafeString?, rowid: Int64) {
        ...
        fn(ndb, RowUpdate(rowid: rowid, op: RowChangeType(op)))
    }
}

public extension HookBox {
    
    static func standard(db: NeXTBase) -> HookBox {
        HookBox(ndb: db) { db, _ in
            db?.lastUpdated = Date()
        }
    }

    static func debug(db: NeXTBase) -> HookBox {
        HookBox(ndb: db) { db, rec in
            db?.lastUpdated = Date()
            print(rec)
        }
    }
...
extension NeXTBase {
    ...
    public func setUpdateHook(_ hook: HookBox? = nil) {
        let hook = hook ?? .standard(db: self)
